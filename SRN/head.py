...
class PVAM(nn.Module):
    def __init__(self, in_channels, char_num, max_text_length, num_heads,
        ...
        self.fc0 = nn.Linear(in_features=in_channels, out_features=in_channels)
        self.emb = nn.Embedding(num_embeddings=self.max_length, embedding_dim=in_channels)
    ...
    def forward(self, inputs, encoder_word_pos, gsrm_word_pos):
        ...
        conv_features = torch.reshape(inputs, (-1, c, h * w))
        ...
        attention_weight = torch.reshape(attention_weight, (-1, self.max_length, t))
...
class GSRM(nn.Module):
    def __init__(self, in_channels, char_num, max_text_length, num_heads,
        ...
        self.mul = lambda x: torch.matmul(x, self.wrap_encoder0.prepare_decoder.emb0.weight.permute(1,0))
    ...
    def forward(self, inputs, gsrm_word_pos, gsrm_slf_attn_bias1,
        ...
        word_ids = torch.reshape(word_ids, (-1, t, 1))
        ...
        gsrm_feature2 = F.pad(gsrm_feature2, (0,0,0,1), value=0.)
...
class VSFD(nn.Module):
    def __init__(self, in_channels, pvam_ch, char_num):
    ...
    def forward(self, pvam_feature, gsrm_feature):
        ...
        img_comb_feature_map = torch.reshape(
            img_comb_feature_map, (-1, t, c1))
        combine_feature = img_comb_feature_map * pvam_feature + (
            1.0 - img_comb_feature_map) * gsrm_feature
        img_comb_feature = torch.reshape(combine_feature, (-1, c1))

        out = self.fc1(img_comb_feature)
        return out

class SRNHead(nn.Module):
    def __init__(self, in_channels = 512, out_channels = 245, max_text_length = 254, num_heads = 8,
                 num_encoder_TUs = 2, num_decoder_TUs = 4, hidden_dims = 512, **kwargs):
        ...
        self.gsrm = GSRM(
            ...
            hidden_dims=self.hidden_dims)
        self.vsfd = VSFD(in_channels=in_channels, pvam_ch=in_channels, char_num=self.char_num)
    ...
    def forward(self, inputs, others):
        ...
        final_out = final_out.view(batch_size, self.max_length, self.char_num)
        decoded_out = decoded_out.view(batch_size, -1)
        word_out = word_out.view(batch_size, self.max_length, self.char_num)
        gsrm_out = gsrm_out.view(batch_size, self.max_length, self.char_num + 1)
